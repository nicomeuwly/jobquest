...
export default function InputElement({ placeholder, type, required, colSpan, value }: InputProps) {
    ...
    return (
        <div className={"relative" + (colSpan ? ` col-span-${colSpan}` : "")}>
            <input type={type} id="floating_outlined" className={"block px-2.5 pb-2.5 pt-4 w-full bg-transparent rounded-lg border-1 border-gray-500 appearance-none focus:outline-none focus:ring-0 focus:border-blue-600 peer hover:bg-gray-50 " + (inputValue ? "text-black" : "text-gray-500")} placeholder=" " name={name} required={required} value={inputValue} onChange={handleChange}/>
